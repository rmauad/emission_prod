...
use "$data/Panel.dta", clear
keep id_uelm year m000a h001d h114d q100a k412a o605 o606 h001a e17 o511a o511 a211a a131a j011a
...
************************
* Creating PPI variable
************************

g ppi = 100 if year == $year3
replace ppi = 79.43 if year == $year2
replace ppi = 68.17 if year == $year1

**************************
* Deflating all variables
**************************

replace y = y*(100/ppi)
replace k = k*(100/ppi)
replace energ = energ*(100/ppi)
replace cont_prev = cont_prev*(100/ppi)
replace environ_prot = environ_prot*(100/ppi)
replace inv = inv*(100/ppi)
replace va = va*(100/ppi)
replace wb = wb*(100/ppi)

*******************************************************************************
* Creating variables to estimate the production function and other regressions
*******************************************************************************
...
scalar alppha = 0.4
// g ln_y_kl = ln(y) - alppha*ln(k) - (1-alppha)*ln(hours)
// g ln_tfp = ln_y_kl + (epsilon/(epsilon-1))*(ln(wb/y) - ln(1-alppha)*(1-gamma))
g ln_tfp =  ln(y) - alppha*ln(k) - (1-alppha)*ln(hours)

* Assuming the production function as in Hassler et al. (2021)
scalar ggamma = 0.5
scalar epsilon = 0.02
g ln_tfp_hassler = ln(y) - alppha*ln(k) - (1-alppha)*ln(hours) + (epsilon/(epsilon-1))*(ln(wb/y) - ln(1-alppha) - ln(1-ggamma)) // this is At in the paper
...
by id_uelm: g dln_lab_prod_lag2 = dln_lab_prod[_n-2]
by id_uelm: g dln_cap_prod_lag2 = dln_cap_prod[_n-2]
by id_uelm: g dln_tfp_lag2 = dln_tfp[_n-2]
by id_uelm: g dln_tfp_hassler_lag2 = dln_tfp_hassler[_n-2]
...
by id_uelm: g ln_lab_prod_lag2 = ln_lab_prod[_n-2]
by id_uelm: g ln_cap_prod_lag2 = ln_cap_prod[_n-2]
by id_uelm: g ln_tfp_lag2 = ln_tfp[_n-2]
by id_uelm: g ln_tfp_hassler_lag2 = ln_tfp_hassler[_n-2]
...
by id_uelm: g dln_ener_prod_lag2 = dln_ener_prod[_n-2]

by id_uelm: g ln_ener_prod_lag1 = ln_ener_prod[_n-1]
by id_uelm: g ln_ener_prod_lag2 = ln_ener_prod[_n-2]
...
save "$data/Panel_complete_feb2025.dta", replace
