...
// Same as reg_iv_nov22, but without the EME dummy
...
esttab using ep_lev_noinst.tex, replace label se ar2 stats(N) star(* 0.10 ** 0.05 *** 0.01) drop(*.year _cons) 
...
esttab using ep_lev_inst1.tex, replace label se ar2 stats(N F) star(* 0.10 ** 0.05 *** 0.01) drop(*.year _cons) scalar(F)

bootstrap, reps(200): ep2sls
esttab using ep_lev_inst2.tex, replace label se ar2 stats(N) star(* 0.10 ** 0.05 *** 0.01) drop(*.year _cons)
