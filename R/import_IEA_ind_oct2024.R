...
iea_sel <- iea_sel %>% filter(!is.na(COU), !is.na(Sectors), (Year >= 2005 & Year <= 2015), Status == "In force" | Status == "Ended") %>%
  mutate(COUyear = paste(COU, Year, sep = "_")) %>%
  select(COUyear, Year, COU, Status, Sectors)

iea_pol_ind <- stringdist_join(iea_sel, iea_ind, by = "Sectors", mode = "left", ignore_case = TRUE, method = "soundex")
...
iea_pol_cum <- iea_pol %>%
  ...
  ungroup() %>%
  select("COUyearind", "Sectors", "iea_pol_cum", "iea_pol_cum_gr")

iea_pol_ind <- iea_pol_cum #changing the name to use in the following code
